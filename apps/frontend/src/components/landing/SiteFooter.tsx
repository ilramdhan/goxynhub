...
export function SiteFooter({ footerNavigation, settings = {} }: SiteFooterProps) {
  const footerText = settings.footer_text || "© 2024 GrowthOS. All rights reserved.";
  const twitterUrl = settings.social_twitter || "";
  const linkedinUrl = settings.social_linkedin || "";
  const githubUrl = settings.social_github || "";

  const footerLinks = footerNavigation?.items?.filter((item) => item.is_active) || [];
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="flex flex-col items-center md:items-start gap-2">
            <Link href="/" className="flex items-center gap-2.5 group">
              ...
              <span className="font-bold text-white text-lg tracking-tight">GrowthOS</span>
            </Link>
            <p className="text-sm text-gray-500 max-w-xs text-center md:text-left">
              The all-in-one platform for scaling your business with confidence.
            </p>
          </div>
        ...
        <div className="mt-8 pt-8 border-t border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 text-xs text-gray-600">
            ...
            <span className="text-indigo-400 font-medium">GrowthOS CMS</span>
          </div>
        </div>
      </div>
    </footer>
